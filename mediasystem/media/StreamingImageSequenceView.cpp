...
#include "StreamingImageSequenceView.h"
...
namespace mediasystem {
    ...
    void StreamingImageSequenceView::step(double timestep)
    {
            ...
            while(true){
                ...
                if(success){
                    ...
                    if(retrieve.img){
                        ...
                        MS_PROFILE_START(load_current_image);
                        mCurrentImage.allocate(retrieve.img->getPixels());
                        MS_PROFILE_STOP(load_current_image);
        ...
        MS_PROFILE_STOP(streaming_imgseq_step);
    }
    
    StreamingImageSequenceView::~StreamingImageSequenceView()
    {
        stopDataLoadingThread();
    }

    StreamingImageSequenceView::StreamingImageSequenceView(SceneBase* scene, float framerate, const Playable::Options& options) : ImageSequenceViewBase(scene, framerate, options)
    {
        mLoadingState = mState;
        startDataLoadingThread();
    }
    
    StreamingImageSequenceView::StreamingImageSequenceView(SceneBase* scene, const std::filesystem::path & source,  float framerate, const Playable::Options& options) : ImageSequenceViewBase(scene, framerate, options)
    {
        mLoadingState = mState;
        initPaths(source);
        startDataLoadingThread();
    ...
    void StreamingImageSequenceView::init()
    {
        mCurrentImage.clear();
        reset();
        //load first frame
    ...
    void StreamingImageSequenceView::start(){
    ...
    void StreamingImageSequenceView::draw()
    {
        if(!mCurrentImage.isAllocated()) return;
        mCurrentImage.draw(0,0);
    }
    
    void StreamingImageSequenceView::debugDraw()
    {
        if(!mCurrentImage.isAllocated()) return;
        //push
        ...
        ofFill();
        ofSetColor(0, 0, 0);
        auto area = "[x: "+ofToString(getPosition().x)+" y: "+ofToString(getPosition().y)+" w: "+ofToString(mCurrentImage.getWidth())+" h: "+ofToString(mCurrentImage.getHeight())+"]";
    ...
    ofTexture* StreamingImageSequenceView::getCurrentTexture()
    {
        return &mCurrentImage;
    }
    
    void StreamingImageSequenceView::setLoop(const bool flag)
    {
        mState.setLoop(flag);
        flushQueue();
    }
    
    void StreamingImageSequenceView::setReverse(const bool flag)
    {
        mState.setReverse(flag);
        flushQueue();
    }
    
    void StreamingImageSequenceView::setBoomarang(const bool flag)
    {
    ...
    void StreamingImageSequenceView::setLoopPoints( uint32_t in, uint32_t out )
    {
    ...
    void StreamingImageSequenceView::setLoopPointIn( uint32_t frame )
    {
    ...
    void StreamingImageSequenceView::setLoopPointOut( uint32_t frame )
    {
        mState.loopPointOut = frame;
        if(mState.isLooping())
            flushQueue();
    ...
    void StreamingImageSequenceView::setLoop( const bool flag, uint32_t inFrame, uint32_t outFrame )
    {
        mState.setLoop(flag);
    ...
    void StreamingImageSequenceView::flushQueue()
    {
        {
    ...
    void StreamingImageSequenceView::startDataLoadingThread()
    {
        if(!mDataLoadingThreadRunning && !mDataLoadingThread){
            mDataLoadingThreadRunning = true;
            mDataLoadingThread.reset( new std::thread(&StreamingImageSequenceView::dataLoadingThread, this));
        }
    }
    
    void StreamingImageSequenceView::stopDataLoadingThread()
    {
        if(mDataLoadingThreadRunning && mDataLoadingThread){
    ...
    void StreamingImageSequenceView::dataLoadingThread()
    {
