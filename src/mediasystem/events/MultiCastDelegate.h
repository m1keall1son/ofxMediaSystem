...
namespace SA {
	...
	public:
        ...
        multicast_delegate& operator -=(const multicast_delegate& another) {
            for (auto& item : another.invocationList){ // clone, not copy; flattens hierarchy:
                auto found = std::find_if(this->invocationList.begin(), this->invocationList.end(), [&item](typename delegate_base<RET(PARAMS...)>::InvocationElement * elem){
                        return *elem == item.invocation;
                });
                if(found != this->invocationList.end()){
                    this->invocationList.erase(found);
                }
            }
            return *this;
        } //operator -=
        ...
        multicast_delegate& operator -=(const delegate<RET(PARAMS...)>& another) {
            if (another.isNull()) return *this;
            auto found = std::find_if(this->invocationList.begin(), this->invocationList.end(), [&another](typename delegate_base<RET(PARAMS...)>::InvocationElement * elem){
                return *elem == another.invocation;
            });
            if(found != this->invocationList.end()){
                this->invocationList.erase(found);
            }
